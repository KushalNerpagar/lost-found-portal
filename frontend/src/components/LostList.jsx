...
function LostList() {
  ...
  const handleResolve = async (id) => {
    ...
    try {
      await fetch(`http://localhost:5000/lost-items/${id}/resolve`, { method: 'PATCH' })
      fetchItems(search)
    } catch {
  ...
  return (
    <section className="rounded-lg bg-white p-5 shadow">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="text-xl font-semibold text-slate-800">📋 Lost Items</h2>
        <input
          className="w-64 rounded-md border border-slate-300 px-3 py-1.5 text-sm outline-none focus:border-blue-500"
      ...
      {!loading && !error && items.length === 0 && <p className="text-slate-500">No lost items found.</p>}
